"""Supabase-klient og dataaksess for Gruppeoppgave 3 (memory.md).

Implementerer PRD §DM-5.2 (felles `kurs`-schema med tabell-prefix per modul)
og §NFR-4.2 (RLS / minimal returning).
...
# PRD §DM-5.2: felles schema, tabell-prefix per modul.
SCHEMA = "kurs"
TABLE = "gruppeoppgave_3_responses"
...
def delete_response(response_id: int) -> None:
    service_client().table(TABLE).delete().eq("id", response_id).execute()
