"""Deltakerflyt for Gruppeoppgave 1 (workshop om AGENTS.md).
...
def main() -> None:
    ...
    crumb(["Kursmoduler", "14 · Gruppeoppgave 1"])
    ...
    st.divider()
    total_sent = sum(st.session_state.get(f"count_{qid}", 0) for qid in QUESTIONS)
    st.caption(f"Du har sendt inn totalt {total_sent} svar i denne sesjonen.")

    st.divider()
    next_module_cta_for("pages/gruppeoppgave_1_resultater.py")
