...
- **v0.67** – Kode-assistenter (modul 02): siden strukturert rundt eierens essens. Fra tre stablede diagrammer uten overskrifter til en 4-takts fortelling med `st.subheader`: «Fra oppgave til handling» -> «En modell alene kan bare skrive» -> «Løsningen: en dialog av verktøykall» -> «Dette kalles Tool Use». De to steg-sekvensene (tre steg kontekst/plan/handling; fire ledd i verktøy-løkka) rendres som `numbered_steps()`-kort matet fra `## `-splittet content via `load_split_markdown` (samme mønster som `skills_md`). Diagrammene `assistant.png`/`llm.png`/`flyt.png` beholdt som visuell støtte (`assistant.png` som lead-bilde). Ny avsluttende Tool Use-seksjon: `callout(info, "Tool Use")` + `callout(tip)` kort bro om at Claude-modellene (Opus/Sonnet/Haiku) er sterke på verktøybruk. Nye/omformatterte content-filer (`steg_oppgave`, `steg_note`, `overgang_sporsmaal`, `steg_lokke`, `tool_use`, `claude_bro`; `bilde_assistant` -> kort lead-tekst); prosa flyttet/omformattert fra eierens bildetekster + essens, ordlyd til eier-bekreftelse. Dekkes av FR-3.11 + FR-3.12; ingen ny FR/NFR. Verifisert: py_compile + AppTest (m02) - 4 subheaders, 2 numbered_steps, 3 bilder, callouts, 0 placeholder.
- **v0.66** – Cortex Code (modul 03): fjernet to oppslag fra «Begrep for begrep» etter eier-beslutning - «Agent-building tasks» (`term_agent_building`) og «Consistent» (`term_consistent`). Tatt ut av `TERM_FILES` i `modules/cortex_code/app_logic.py` og de to nå foreldreløse `content/*.md`-filene slettet (git-sporet, reverserbart). 5 begrep igjen (intelligent, intelligent_agent/AI agent, autonomous, best_practices, context_aware). Dekkes av FR-3.11 (presentasjons-modul) + FR-3.12 (prosa i content); ingen ny FR/NFR. Verifisert: py_compile + ingen gjenværende referanser.
- **v0.65** – «Hvordan kode assistenter fungerer» skilt ut fra «Under panseret» til **egen modul nr 02** (slug `kodeassistenter`, kategori I), plassert rett etter Evolusjon og før Cortex Code. Pedagogikk: den generelle, produkt-uavhengige mekanikken (språkmodell alene -> assistent med verktøy + loop -> meldingsflyt) læres før det Cortex Code-spesifikke. De tre diagrammene + bildetekstene fra v0.64 (`assistant.png`/`llm.png`/`flyt.png` + `bilde_*.md`) flyttet til `modules/kodeassistenter/content/`; `_image_or_placeholder`-helperen kopiert dit. «Under panseret» (nå nr 05) er trimmet til intro + «Lagene» + demo, og den nå ubrukte helperen fjernet derfra. **Renummerering:** ny modul på plass 2 skjøv gamle nr 2-24 -> 3-25 (25 moduler totalt); 23 wrapper-filer under `pages_content/modules/` renamet + crumb-nr oppdatert i hver berørte `app_logic.py`. SECTIONS-page_ids oppdatert (Introduksjon = evolusjon, kodeassistenter, cortex_code, cortex_interaction, arkitektur). CTA-kjeden er slug-basert og uberørt utover innsettingspunktet: `evolusjon` -> `kodeassistenter` -> `cortex_code`. Dekkes av FR-3.11 + FR-3.12; ingen ny FR/NFR (FR-3.8/FR-3.10 er fra før utdatert, ikke patchet). Verifisert: smoke-import 25 wrappere (0 orphans), crumb==nr, py_compile 0 feil.
- **v0.64** – Under panseret (modul 04): ny seksjon «Hvordan kode assistenter fungerer» lagt inn mellom intro og «Lagene». Viser tre forklarende diagrammer stablet i full bredde - `assistant.png` (kode-assistent = språkmodell + verktøy i en iterasjons-løkke), `llm.png` (språkmodell alene, uten assistent, kan ikke lese filer) og `flyt.png` (sekvensdiagram for et `ReadFile`-verktøykall med assistent) - hver med en kort norsk bildetekst (eier-bedt draft i `content/bilde_assistant.md` + `content/bilde_llm.md` + `content/bilde_flyt.md`). Rekkefølgen forteller en progresjon: oversikt -> problem (modell alene kan ikke handle) -> løsning (assistenten muliggjør verktøybruk). Bildene flyttet fra repo-rot inn i `modules/arkitektur/content/`. `app_logic.py` fikk lokal `_image_or_placeholder`-helper + `_HERE` (samme mønster som `evolusjon`/`cortex_interaction`). Bildegrafikken er på engelsk; bildetekstene bygger bro til norsk. Dekkes av FR-3.11 (presentasjons-modul) + FR-3.12 (prosa i content); ingen ny FR/NFR. Verifisert: py_compile + AppTest (m04) - ny subheader før «Lagene», 3 `st.image` + 3 `st.caption`, 0 placeholder, 0 feil.
- **v0.63** – Cortex Code (modul 02): fjernet «Lytteklipp: Snowflake RBAC»-seksjonen (`st.audio`) etter eier-beslutning. Tatt ut seksjonen, den foreldreløse `RBAC_AUDIO`-konstanten + repo-rot-path-utregningen, den nå ubrukte `Path`-importen og fallback-callouten i `modules/cortex_code/app_logic.py`; gjenværende seksjoner renummerert (Begrep/Klartekst/Eksempel = 2-4). Lydfila `Snowflake RBAC.mp3` i repo-rot er nå urefererert men ikke slettet. Var dekket av generell FR-3.11 (presentasjons-modul); ingen dedikert FR fjernet, ingen ny FR/NFR. Reverserer plasseringen fra v0.48-changelog. Verifisert: py_compile OK, ingen gjenværende `RBAC.mp3`/`RBAC_AUDIO`/`cortex_audio`-referanser.
- **v0.62** – memory.md-seksjonen skjult fra kurset (eier-beslutning: skal ikke med). De tre modulene memory.md, Gruppeoppgave 3 og Resultater Gruppeoppgave 3 (var nr 21-23) fjernet fra `MODULER` + «memory.md»-SECTIONS i `data/moduler.py`. **Skjult, ikke slettet:** `modules/memory_md/`, `modules/gruppeoppgave_3/`, `modules/gruppeoppgave_3_resultater/` og DB-tabellen `kurs.gruppeoppgave_3_responses` er urørt; de tre wrapperne flyttet til `pages_content/modules/_skjult/` (ut av ruting). De fire påfølgende modulene renummerert 24-27 -> 21-24 (context_engineering, kohort, konkurrent, avslutning) med wrapper-rename + crumb-justering. CTA `gruppeoppgave_2` -> `memory_md` rekoblet til `context_engineering`. 24 moduler igjen. **Gjenåpning:** re-legg de tre radene i MODULER + memory.md-SECTIONS, flytt wrapperne tilbake, rekoble CTA. FR-3.10s nav-spec var fra før utdatert (ikke patchet). Ingen ny FR/NFR. Verifisert: wrapper-integritet (24, 0 orphans), crumb==nr, smoke-import 0 feil.
